...
## ייבוא פרקים מקובץ טקסט (הדרך הקלה)

כותבים את הפרקים ב-Google Docs או ב-Word, ושומרים כטקסט פשוט (או מדביקים לקובץ `.md`):

```
# פרק 1: הכוכב שלא רצה לישון
תאריך: 2026-10-01        (לא חובה)
נושא: ויסות לפני שינה    (לא חובה)
...
```
npm run story:import -- chapters.md --story shira-moon --title "שירה והירח" --start 2026-10-01
```

כל כותרת `# פרק N` פותחת פרק, שורה ריקה מפרידה בין פסקאות. פרק בלי תאריך מקבל את היום הבא בתור, כך שכל ערב נפתח פרק אחד. מזהה הסיפור (`--story`) באותיות אנגליות.
